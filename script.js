/**
 * 🇧🇷 MAPA INTERATIVO DE GÍRIAS BRASILEIRAS - VERSÃO 2.0 PREMIUM
...
function esconderLoadingScreen() {
    ...
    if (loadingScreen) {
        setTimeout(() => {
            ...
            mostrarToast('🇧🇷 Bem-vindo ao Brasil das Gírias v2.0!', 'success');
        }, 1000);
    }
}
...
function compartilharEstado(estadoId) { 
    ...
    if (navigator.share) {
        navigator.share({
            title: `Gírias de ${giriasData[estadoId]?.estado}`,
            text: `Descubra as gírias de ${giriasData[estadoId]?.estado}!`,
            url: url
        });
    } else {
...
function sugerirGiria(estadoId) { mostrarToast('💡 Entre em contato via LinkedIn para sugerir gírias!', 'info'); }
...
function mostrarSobre() { 
    mostrarToast('ℹ️ Projeto educativo sobre a diversidade linguística brasileira v2.0', 'info'); 
}
...
console.log('✅ Sistema Premium de Gírias Brasileiras v2.0 carregado com sucesso! 🇧🇷'); 